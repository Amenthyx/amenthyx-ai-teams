...
|----------|-------------|
| Schema Operations | Create, alter, drop - tables, indexes, constraints |
| Migration | Forward, rollback, data migration, zero-downtime DDL |
| Query Performance | Execution plans, index usage, slow query ID |
| Replication | Primary-replica sync, lag monitoring, failover |
| Backup/Restore | Full, incremental, point-in-time, verification |
| Transaction | ACID compliance, isolation levels, deadlock detection |
| Security | User privileges, row-level security, audit logging |
| High Availability | Cluster health, automatic failover, quorum |
