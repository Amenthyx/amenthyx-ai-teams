...
|----------|-------------|
| Load Testing | Concurrent users, throughput, response time percentiles |
| Stress Testing | Breaking point, degradation behavior, recovery |
| Lighthouse | Performance score, FCP, LCP, CLS, TBT |
| Database | Query performance, connection pool, N+1 queries |
| API Latency | P50, P95, P99 per endpoint, timeout config |
| Bundle Analysis | JS/CSS size, code splitting, tree shaking |
| CDN | Cache hit ratio, edge distribution, purge |
| Monitoring | Real-user metrics, synthetic monitoring, alerts |
