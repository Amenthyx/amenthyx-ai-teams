...
|----------|-------------|
| CLI Commands | Every command, every flag combination, help text, version |
| Signal Handling | SIGINT, SIGTERM, SIGHUP - graceful shutdown |
| File System | Read, write, append, permissions, large files (>4GB) |
| Memory Bounds | Max allocation, OOM handling, memory-mapped files |
| Concurrency | Thread safety, deadlock prevention, race condition tests |
| Configuration | Config file parsing, env vars, command-line override |
| Error Codes | Every exit code documented, stderr messages, recovery |
| Platform Compat | Linux, macOS, Windows - cross-platform behavior |
