...
|----------|-------------|
| API Endpoints | Every route, HTTP method, status code |
| Authentication | JWT issue, refresh, revoke, middleware enforcement |
| Authorization | Role-based, resource-based, ownership checks |
| Validation | Request body, query params, path params, headers |
| Error Responses | 400, 401, 403, 404, 409, 422, 429, 500 - correct body |
| Rate Limiting | Per-user, per-IP, per-endpoint, burst handling |
| WebSocket | Connect, disconnect, reconnect, message ordering |
| Caching | Cache hit/miss, invalidation, TTL |
