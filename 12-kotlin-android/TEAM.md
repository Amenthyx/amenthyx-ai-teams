...
|----------|-------------|
| Touch Gestures | Tap, long-press, swipe, pinch, double-tap, drag |
| App Lifecycle | Background, foreground, kill, restart, deep link restore |
| Push Notifications | Receive, display, tap-to-navigate, dismiss, badge count |
| Orientation | Portrait, landscape, rotation during form fill |
| Offline Mode | Queue operations, sync on reconnect, cached data display |
| Biometrics | Face ID, Touch ID, fingerprint, fallback to PIN |
| Deep Links | Universal links, app links, deferred deep links |
| In-App Purchase | Subscription flow, restore purchases, receipt validation |
